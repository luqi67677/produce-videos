---
name: produce-videos
description: 把讲稿、文档、截图、图片、PPT 或录屏制作成 16:9 横屏或 3:4 竖屏动态视频和独立封面。按风险把相关内容合并成少量确认包，同时保留口播、真实素材、声音、分镜、动态预览和成片的可验证质量门；可复用本地 TTS，缺失时经用户授权准备 Qwen3-TTS。适用于完整制作、续作和定点修改；用户只要求脚本、分镜或审查时不擅自渲染。
---

# 2026-09-05 视频生成与编辑 V2.0.1
...
1. 复用用户已经说明的信息，只补问会实质改变结果的缺项；用户只要求脚本、分镜或审查时停在该步。
2. 先核验事实与口播，不让视觉替内容背书；本地输出、上传、草稿、发布和真实用户效果分开表述。
3. 用户没有视觉方向时，先验证完整 34 套模板目录并对全部模板评分，再输出三套同内容、同画幅的真实候选；用户已有视觉系统时直接验证并锁定。
4. 候选内容帧达到校准质量，选中后成为全片唯一版式基线；封面、字幕、分镜、动画和成片共同读取 `video-style-theme.json`。
5. 用户确认的标题逐字保留；只有明确批准另一封面标题时才能改写。中文混排使用中文优先字体并保持品牌原始大小写。
...
| 模式 | 适用条件 | 用户确认节奏 |
|---|---|---|
| `fast` | 通常不超过 30 秒；事实稳定；无敏感录屏或授权疑点；使用成熟主题和运动语法 | 用户已给全信息时跳过启动追问，默认 4 个确认包 |
| `standard` | 普通宣传片、教程或混合素材；需要 TTS；风险可控 | 默认 4—5 个确认包 |
| `high-risk` | 敏感录屏、授权不明、强事实主张、超过 90 秒、首次运动语法或需要下载安装 | 拆出隐私/授权、下载授权或短动态样片 |
...
读取 `assets/format-presets.json`，从模板建立 `video-brief.md`。

- 用户已说明画幅、平台、时长、素材、音频路线或声音方向时直接记录，不重复确认。
- 缺少多个信息时在一个问题中补齐，不逐项发问。
- 记录 `workflow_mode`、判断理由、禁用词、隐私目标、授权边界和输出目录。
- 不因建立 brief 自动扫描、下载、调用云端服务或生成下游产物。

### 2. 内容方向包

按需读取 `references/content-to-video-orchestration.md`、`references/director-workflow.md` 和 `references/visual-style-system.md`。

1. 从资料建立 `claims-ledger.md` 和完整 `master-script.json`；区分事实、推断、演示数据和待核验项。
2. 建立 `source-assets.json`，记录真实文件、SHA-256、来源、授权、用途、隐私目标和联系表；没有真实素材时记录生成边界。
3. 验证 34 套模板目录。机器侧为全部模板记录分数、结论和简短理由码，只为入围 A/B/C 写详细理由。
...
`fast/standard` 把完整口播、真实素材联系表和三套风格放入一个确认包。提示用户按“内容和素材通过，选 B”回复；单独的 `B` 只表示选择风格，不自动批准口播和素材。`high-risk` 先单独确认隐私、授权和禁止使用项。
...
需要 TTS 时读取 `references/audio-system.md`。

- 已有成品口播：登记实际文件及技术信息，不增加试听步骤。
- 已有 TTS API：只通过环境变量或安全存储读取凭证，最小预检后按声音方向生成试听。
- 已有本地模型：用户选择该路线后才有限发现模型和兼容 Python。唯一兼容候选直接推荐并预检；多个能力差异明显的候选才让用户选择。
- 没有可用方案：说明 Qwen3-TTS 地址、预计大小、目标目录和依赖，单独取得授权后才安装或下载。
...
按需读取 `references/layout-system.md`、`references/director-workflow.md` 和 `references/implementation.md`。

- 完成全部镜头素材、独立封面草图、`asset-manifest.md`、素材复用关系、`shot-readiness.json` 和全量静态分镜。
- 一段口播只绑定一个镜头；每个镜头写明主素材、开始/变化/结束状态、可见证据、录屏入出点、运动语法和隐私复核。
...
- `fast/standard`：通过 sample 门禁后直接生成完整低清预览。
- `high-risk` 或首次运动语法：先做 4—8 秒、只覆盖最高风险动作的最短样片；通过后再生成完整低清预览。
...
项目至少保留：brief、主张账本、完整口播、真实素材清单、视觉候选、唯一主题、声音需求与选择、旁白契约、素材清单、镜头就绪表、运动覆盖、V2 审批账本、分镜、风险样片（如需）、完整低清预览、正式视频、质检报告和联系表。

每条完整视频都交付一张 1080×1440 独立封面和 270×360 缩略图。读取 `assets/cover-brief-template.md`，使用锁定主题原生重构，不用横屏首帧裁切代替。
## 校验命令
...
- 不用固定三套、空模板、色块或低质量样张冒充 34 套内容匹配，也不混合不同模板的设计语言。
