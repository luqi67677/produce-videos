...
When the user has not supplied a visual direction, preview three candidates:

- one `balanced` option from `selection-index.json`
- one `expressive` option from `selection-index.json`
- one `unexpected-fit` option from `selection-index.json`

If the user has already supplied a visual system or explicitly selected a
template, validate and lock that direction instead of forcing three previews.

Adjust the tone inside that candidate mix:
