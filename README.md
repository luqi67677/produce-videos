...
| 4. 完整分镜 | 审查原始素材，标记 A-roll/B-roll，匹配 88 套布局，为每个剪辑镜头绑定口播、证据、人物动作和语义切点 | 不再用章节代替镜头；逐句确认画面、动作、切换和素材来源 |
| 5. 动态证明 | 生成 4—8 秒代表性微样片，并对完整低清预览执行声画同步、逐镜运动和空白帧审计 | 静态持有、等待入场的空白、运动计划漂移都会阻止正式导出 |
| 6. 成片 | 生成完整低清预览，确认后再导出正式视频、独立封面和可选时间线 | 封面另验主副标题、Skill 名称、主视觉动作和缩略图 |
...
| 7. 分镜 | 原始素材审片计划、人物档案、逐镜口播切点、完整静态分镜和平台遮挡图 | 每句话对应什么画面、怎么动、何时切换都能看见 |
| 8. 动态证明与成片 | 代表性微样片、完整低清预览、逐镜审计、正式视频和独立封面 | 同时验证运动适量、首帧不空、声画不串页和封面层级 |
...
布局还带有硬限制：一个焦点、有限的辅助组与强调色、单一材质、画面占用率和最大空白区。每镜第一帧必须先有稳定骨架或主视觉，不能清空画面等待动画入场；脏色、噪点、随机粒子、发光 AI 大脑、漂浮图标、重色字幕条和裁切残留会直接阻止渲染。详细规则见 [视觉干净度系统](references/visual-cleanliness-system.md)。

## A-roll / B-roll 与素材补缺

A-roll 是主叙事画面，B-roll 是在同一声音主轴上切入的证据、操作演示或概念解释。B-roll 不是装饰；如果不能指出它解释的口播，就不应该出现。V2.5.0 起，`shot-readiness.json`、EDL 和运动计划必须使用同一组逐镜口播片段与语义切点，避免上一句跑到下一张画面。
...
需要反复出现同一人物或角色时，先建立 `character-profile-*.json`，锁定身份锚点、脸型、发型、体型、服装、色板、画风、允许变化和禁止变化。同一个 `character_id` 必须贯穿 A-roll、B-roll、封面、静态补图和动态样片；每个含人物镜头还要单独定义场景动作、动作签名和服装语境。未经说明的换脸、换衣、画风漂移，或把同一姿势复制到不同职责的镜头，都会阻止渲染。

## 逐镜门禁与封面门禁

V2.5.0 不再只检查章节级 `scenes`。新项目在 `shot-readiness.json` V2.6 中逐一登记真正进入时间线的剪辑镜头，并与 EDL V1.1 对齐：口播片段、起止时间、A/B-roll 职责、真实审查帧、证据、版式、观众文案和人物动作都必须可追溯。制作说明、马赛克说明和“真实素材”之类内部标签不得进入观众画面。

完整预览还会检查三种失败：画面只有轻微假动效、列表没有按口播逐项出现，以及画面先空着等待元素入场。独立封面则通过 `cover-contract.json` 单独检查主标题、副标题、Skill 名称、主题绑定、人物动作和缩略图可读性。
